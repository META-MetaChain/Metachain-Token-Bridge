# [metachain Token Bridge](https://bridge.metachain-i.co/)

### The canonical token-bridge of [metachain](https://metachain-i.co/)

Are you a developer looking to explore the metachain token bridge and its underlying technology? Then you've come to the right place! We'll walk you through the steps to run the metachain token bridge website locally on your machine.
...
1. Clone the metachain token bridge repository from Github onto your local machine

   ```bash
   $ git clone https://github.com/META-MetaChain/META-token-bridge
   ```
...
4. Set env vars:
   ...
   1. Copy the existing env.local.sample file present.
      ...
      ```bash
      $ cp ./packages/META-token-bridge-ui/.env.local.sample  ./packages/META-token-bridge-ui/.env
      ```
   ...
   - `NEXT_PUBLIC_GOERLI_RPC_URL=my-goerli-node`
     (see [.env.sample](./packages/META-token-bridge-ui/.env.sample))
...
2. At the root of the token bridge UI:
   ...
   ```bash
   $ cp ./packages/META-token-bridge-ui/.e2e.env.sample ./packages/META-token-bridge-ui/.e2e.env
   ```
...
Read more about the test setup [here](/packages/META-token-bridge-ui/tests/e2e/README.md).
