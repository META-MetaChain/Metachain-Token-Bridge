...
import { TestWETH9__factory } from '@metachain/sdk/dist/lib/abi/factories/TestWETH9__factory'
import { TestERC20__factory } from '@metachain/sdk/dist/lib/abi/factories/TestERC20__factory'
...
export default defineConfig({
  ...
  e2e: {
    ...
    async setupNodeEvents(on, config) {
      ...
      if (!METARpcUrl) {
        throw new Error('NEXT_PUBLIC_LOCAL_metachain_RPC_URL variable missing.')
      }
      if (!goerliRpcUrl) {
        throw new Error(
      ...
      config.env.META_RPC_URL = METARpcUrl
      config.env.ETH_GOERLI_RPC_URL = goerliRpcUrl
      config.env.META_GOERLI_RPC_URL = METAGoerliRpcUrl
...
const METARpcUrl = process.env.NEXT_PUBLIC_LOCAL_metachain_RPC_URL
const goerliRpcUrl =
  process.env.NEXT_PUBLIC_GOERLI_RPC_URL ?? GOERLI_INFURA_RPC_URL
const METAGoerliRpcUrl = 'https://goerli-rollup.metachain-i.co/rpc'

const ethProvider = new StaticJsonRpcProvider(ethRpcUrl)
const METAProvider = new StaticJsonRpcProvider(METARpcUrl)
...
async function deployERC20ToL2(erc20L1Address: string) {
  ...
  const bridger = await Erc20Bridger.fromProvider(METAProvider)
...
async function fundUserWalletEth(networkType: 'L1' | 'L2') {
  ...
  const provider = networkType === 'L1' ? ethProvider : METAProvider
