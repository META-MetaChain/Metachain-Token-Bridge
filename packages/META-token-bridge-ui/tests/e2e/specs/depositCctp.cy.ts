...
describe('Deposit USDC through CCTP', () => {
  ...
  context('User has some USDC and is on L1', () => {
    ...
    beforeEach(() => {
      ...
      getInitialERC20Balance({
        tokenAddress: CommonAddress.metachainGoerli['USDC.e'],
        multiCallerAddress: getL2TestnetNetworkConfig().multiCall,
        rpcURL: Cypress.env('META_GOERLI_RPC_URL'),
        address
      }).then(
    ...
    it(
      ...
      () => {
        ...
        context('should show L1 and L2 chains, and USD correctly', () => {
          ...
          cy.findByRole('button', { name: /To: metachain Goerli/i }).should(
            'be.visible'
          )
        ...
        context('should show clickable deposit button', () => {
          cy.findByRole('button', {
            name: /Move funds to metachain Goerli/i
          })
            .should('be.visible')
            .should('be.enabled')
            .click()
        ...
        context('Should display CCTP modal', () => {
          // Tabs
          cy.findByRole('tab', {
            name: "metachain's bridge (USDC.e)",
            selected: true
          }).should('exist')
          cy.findByRole('tab', {
