...
import { CommonAddress } from 'packages/META-token-bridge-ui/src/util/CommonAddressUtils'
...
describe('Withdraw USDC through CCTP', () => {
  ...
  context('User is on L2 and imports USDC', () => {
    ...
    beforeEach(() => {
      ...
      getInitialERC20Balance({
        tokenAddress: CommonAddress.metachainGoerli.USDC,
        multiCallerAddress: getL2TestnetNetworkConfig().multiCall,
        rpcURL: Cypress.env('META_GOERLI_RPC_URL'),
        address
      }).then(
        val =>
    ...
    it(
      ...
      () => {
        cy.login({ networkType: 'L2', networkName: 'metachain-goerli' })
        context('should show L1 and L2 chains, and ETH correctly', () => {
          cy.findByRole('button', { name: /From: metachain Goerli/i }).should(
            'be.visible'
          )
        ...
        context('should add USDC token', () => {
          ...
          cy.findByPlaceholderText(/Search by token name/i)
            .typeRecursively(CommonAddress.metachainGoerli.USDC)
            .should('be.visible')
            .then(() => {
