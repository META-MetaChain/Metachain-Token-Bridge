...
describe('Withdraw ETH', () => {
  ...
  context('user has some ETH and is on L2', () => {
    it('should show form fields correctly', () => {
      ...
      cy.findByRole('button', { name: /From: metachain/i }).should('be.visible')
        ...
        typeAmountIntoInput()
          ...
          .then(() => {
            cy.findByRole('button', {
              ...
              .should('be.enabled')
              .click()
            cy.findByText(/Use metachain’s bridge/i).should('be.visible')
  ...
  // 1. metachain network is not added to metamask yet (add + switch)
  // 2. metachain network already configured in metamask (only switch)
