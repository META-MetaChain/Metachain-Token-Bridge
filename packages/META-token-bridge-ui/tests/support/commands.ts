...
const METAGoerliRpcUrl = Cypress.env('META_GOERLI_RPC_URL')
const goerliProvider = new StaticJsonRpcProvider(goerliRpcUrl)
const METAGoerliProvider = new StaticJsonRpcProvider(METAGoerliRpcUrl)
...
export async function resetCctpAllowance(networkType: 'L1' | 'L2') {
  const provider = networkType === 'L1' ? goerliProvider : METAGoerliProvider
  const { USDC, tokenMessengerContractAddress } =
    networkType === 'L1' ? CommonAddress.Goerli : CommonAddress.metachainGoerli
...
export async function fundUserUsdcTestnet(networkType: 'L1' | 'L2') {
  ...
  const usdcBalance = await getInitialERC20Balance({
    ...
    rpcURL: networkType === 'L1' ? goerliRpcUrl : METAGoerliRpcUrl,
