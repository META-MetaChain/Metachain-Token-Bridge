...
import { MultiCaller } from '@metachain/sdk'
...
export const getL2NetworkConfig = (): NetworkConfig => {
  return {
    networkName: 'metachain-localhost',
    rpcUrl: Cypress.env('META_RPC_URL'),
    chainId: '412346',
    symbol: 'ETH',
    isTestnet: true,
...
export const ERC20TokenName = 'IntMETATestToken'
export const ERC20TokenSymbol = 'IMETA'
...
export const startWebApp = (url = '/', qs: { [s: string]: string } = {}) => {
  ...
  cy.clearLocalStorage('metachain:bridge:tos-v2')
  cy.visit(url, {
    qs
  })
