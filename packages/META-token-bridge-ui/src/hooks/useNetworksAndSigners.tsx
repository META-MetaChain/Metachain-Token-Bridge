...
1. Is the network connected? If yes, is it metachain (L2) or Ethereum (L1)?
...
import { getChain, getParentChain } from '@metachain/sdk'
...
import { useIsConnectedTometachain } from './useIsConnectedTometachain'
import {
...
export function NetworksAndSignersProvider(
  ...
  const update = useCallback(async () => {
    ...
    /***
     * Case 1: Connected to an Orbit chain & metachain is 'preferredL2ChainId.' Need to set 'preferredL2ChainId' to an Orbit chain.
     ...
     */
    const isSelectedL2Chainmetachain = isNetwork(selectedL2ChainId!).ismetachain
    let _selectedL2ChainId = selectedL2ChainId
    const providerSupportedL2 = chainIdToDefaultL2ChainId[providerChainId]!

    // Case 1: Connected to an Orbit chain & metachain is 'preferredL2ChainId'. Need to set 'preferredL2ChainId' to an Orbit chain.
    if (isConnectedToOrbitChain && isSelectedL2Chainmetachain) {
      if (thisInvocation !== invocationCounter.current) {
        // don't apply if the iteration is not the latest
        return
      }
    ...
    getParentChain(provider as Web3Provider)
      .then(async parentChain => {
        const isParentChainmetachain = isNetwork(parentChain.chainID).ismetachain

        // There's no preferred L2 set, but metachain is the ParentChain.
        if (isParentChainmetachain && !selectedL2ChainId) {
          // By default, we want metachain to be paired with Ethereum instead of an Orbit chain in our UI.
          // This is so we default users to their preferred case: Withdrawal from metachain to Ethereum.
          // However, the current flow would set it to metachain's partner Orbit chain.

          // We jump to 'getChain'. This means metachain will be considered an L2 and paired with Ethereum.
          throw new Error()
        }
        ...
        if (isParentChainmetachain && isSelectedL2Chainmetachain) {
          // Special case if user selects an Ethereum-Orbit pair in the UI.
          // Before the switch happens, metachain is the preferred L2 chain (in query params), but L1 is also metachain, hence such if statement.

          // We know our 'chain' is metachain, we need to set 'parentChain' to Ethereum (metachain's 'partnerChainID').
      ...
      .catch(error => {
        ...
        if (providerChainId !== _selectedL2ChainId && !isConnectedTometachain) {
          // Make sure the Chain provider chainid match the selected chainid
          // We continue the check if connected to metachain, because metachain can be either ParentChain or Chain.
          setResult({
            status: UseNetworksAndSignersStatus.NOT_SUPPORTED,
            chainId: providerChainId
          })
