...
import { L1EthDepositTransaction } from '@metachain/sdk/dist/lib/message/L1Transaction'
import { ERC20__factory } from '@metachain/sdk/dist/lib/abi/factories/ERC20__factory'
import { EventArgs } from '@metachain/sdk/dist/lib/dataEntities/event'
import { L2ToL1TransactionEvent } from '@metachain/sdk/dist/lib/message/L2ToL1Message'
import { L2ToL1TransactionEvent as ClassicL2ToL1TransactionEvent } from '@metachain/sdk/dist/lib/abi/METASys'
...
import {
  ...
  METATokenBridgeEth,
  METATokenBridgeToken
} from './METATokenBridge.types'
...
// https://github.com/META-MetaChain/metachain-sdk/blob/main/src/lib/message/L1ToL2MessageGasEstimator.ts#L76
...
export const useMETATokenBridge = (
  ...
  const [executedMessagesCache, setExecutedMessagesCache] =
    useLocalStorage<ExecutedMessagesCache>(
      'metachain:bridge:executed-messages',
      {}
    ) as [
  ...
  const withdrawEth: METATokenBridgeEth['withdraw'] = async ({
    amount,
    l2Signer,
    txLifecycle
  }) => {
  ...
  const withdrawToken: METATokenBridgeToken['withdraw'] = async ({
    erc20L1Address,
    amount,
    l2Signer,
    txLifecycle,
  ...
  const addTokensFromList = async (METATokenList: TokenList, listId: number) => {
    ...
    for (const tokenData of METATokenList.tokens) {
      ...
      const bridgeInfo = (() => {
        // TODO: parsing the token list format could be from METAts or the tokenlist package
      ...
      else if (METATokenList.tokens.length < 1000) {
    ...
    // add L1 tokens only if they aren't already bridged (i.e., if they haven't already beed added as L2 META-tokens to the list)
    ...
    setBridgeTokens(oldBridgeTokens => {
      ...
      if (isNetwork(l2ChainID).ismetachainOne) {
        l2Addresses.push(CommonAddress.metachainOne.USDC)
      }
      if (isNetwork(l2ChainID).ismetachainGoerli) {
        l2Addresses.push(CommonAddress.metachainGoerli.USDC)
      }
