...
import { AssetType, TransactionActions } from './METATokenBridge.types'
...
const localStorageReducer = (state: Transaction[], action: Action) => {
  ...
  window.localStorage.setItem('METATransactions', JSON.stringify(stateForCache))
  return newState
}
...
const useTransactions = (): [Transaction[], TransactionActions] => {
  ...
  useEffect(() => {
    const cachedTransactions = window.localStorage.getItem('METATransactions')
    dispatch({
