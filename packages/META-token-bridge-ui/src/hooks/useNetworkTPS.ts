...
const fetchNetworkTPS = async (l2ChainId: number) => {
  // currently we only support TPS information for META-one and nova
  // return null for rest of the networks
  const canFetchTPS =
    isNetwork(Number(l2ChainId)).ismetachainNova ||
    isNetwork(Number(l2ChainId)).ismetachainOne
  ...
  const $ = load(html)
  const tpsElement = $(
    'span[title="Transactions per Second - Adjusted for metachain Nitro system txs"]'
  )
