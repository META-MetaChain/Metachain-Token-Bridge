...
import { getL2Network } from '@metachain/sdk'

import {
  l2METAReverseGatewayAddresses,
...
export async function fetchL2Gateways(l2Provider: Provider) {
  ...
  const l2METAReverseGateway = l2METAReverseGatewayAddresses[l2Network.chainID]
  ...
  if (l2METAReverseGateway) {
    gatewaysToUse.push(l2METAReverseGateway)
  }
  if (l2DaiGateway) {
