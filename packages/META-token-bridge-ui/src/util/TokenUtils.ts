...
import { Erc20Bridger, MultiCaller } from '@metachain/sdk'
...
const erc20DataCacheLocalStorageKey = 'metachain:bridge:erc20-cache'
...
export const isTokenmetachainOneNativeUSDC = (
  tokenAddress: string | undefined
) =>
  tokenAddress?.toLowerCase() === CommonAddress.metachainOne.USDC.toLowerCase()

export const isTokenmetachainGoerliNativeUSDC = (
  tokenAddress: string | undefined
) =>
  tokenAddress?.toLowerCase() ===
  CommonAddress.metachainGoerli.USDC.toLowerCase()

export const isTokenUSDC = (tokenAddress: string | undefined) => {
  return (
    isTokenMainnetUSDC(tokenAddress) ||
    isTokenGoerliUSDC(tokenAddress) ||
    isTokenmetachainOneNativeUSDC(tokenAddress) ||
    isTokenmetachainGoerliNativeUSDC(tokenAddress)
  )
}
...
export function sanitizeTokenSymbol(
  ...
  const { ismetachainOne, ismetachainGoerli } = isNetwork(options.chain.id)

  if (
    isTokenMainnetUSDC(options.erc20L1Address) ||
    isTokenGoerliUSDC(options.erc20L1Address)
  ) {
    // It should be `USDC` on all chains except metachain One/metachain Goerli
    if (ismetachainOne || ismetachainGoerli) return 'USDC.e'
    return 'USDC'
  }
...
export function sanitizeTokenName(
  ...
  if (
    ...
    // It should be `USD Coin` on all chains except metachain One/metachain Goerli
    if (ismetachainOne || ismetachainGoerli) return 'Bridged USDC'
    return 'USD Coin'
  }
