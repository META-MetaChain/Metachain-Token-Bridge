...
import { L2ToL1MessageReader, L2TransactionReceipt } from '@metachain/sdk'
...
import { getExecutedMessagesCacheKey } from '../../hooks/useMETATokenBridge'
...
export async function getOutgoingMessageState(
  ...
  const executedMessagesCache = JSON.parse(
    localStorage.getItem('metachain:bridge:executed-messages') || '{}'
  )
