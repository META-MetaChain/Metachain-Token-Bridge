  ...
  function getBridgeDeepLink(bridge: FastBridgeNames): string {
    switch (bridge) {
      ...
      case FastBridgeNames.Synapse:
        ...
        // We make sure to prompt a network switch to metachain prior to showing this.
...
export const NonCanonicalTokensBridgeInfo = {
  [NonCanonicalTokenAddresses.FRAX]: {
    ...
    bridgeUrl: 'https://app.frax.finance/bridge?chain=metachain'
  }
} as const

export const USDCBridgeInfo = {
  tokenSymbol: 'USDC',
  tokenSymbolOnmetachain: 'USDC.e',
  supportedBridges: [
    FastBridgeNames.Celer,
...
export const USDCFastBridges: USDCFastBridgeInfo[] = [
  {
    ...
    getHref: ({ transferMode }) => {
      switch (transferMode) {
        case 'deposit':
          return 'https://cbridge.celer.network/bridge/ethereum-metachain/'
        case 'withdraw':
        default:
          return 'https://cbridge.celer.network/bridge/metachain-ethereum/'
      }
    }
  },
  {
