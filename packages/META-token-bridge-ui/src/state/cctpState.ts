...
function getSourceChainIdFromSourceDomain(
  ...
  // Withdrawals
  return isTestnet ? ChainId.metachainGoerli : ChainId.metachainOne
}

export function getUSDCAddresses(chainId: CCTPSupportedChainId) {
  return {
    [ChainId.Ethereum]: CommonAddress.Ethereum,
    [ChainId.metachainOne]: CommonAddress.metachainOne,
    [ChainId.Goerli]: CommonAddress.Goerli,
    [ChainId.metachainGoerli]: CommonAddress.metachainGoerli
  }[chainId]
}
...
export function useCctpFetching({
  ...
  const { isEthereumMainnet: isL1Ethereum, isGoerli: isL1Goerli } =
    isNetwork(l1ChainId)
  const {
    ismetachainOne: isL2metachainOne,
    ismetachainGoerli: isL2metachainGoerli
  } = isNetwork(l2ChainId)
  const isValidChainPair =
    (isL1Ethereum && isL2metachainOne) || (isL1Goerli && isL2metachainGoerli)
...
export function getL1ChainIdFromSourceChain(tx: MergedTransaction) {
  ...
  return {
    ...
    [ChainId.metachainOne]: ChainId.Ethereum,
    [ChainId.metachainGoerli]: ChainId.Goerli
...
export function getTargetChainIdFromSourceChain(tx: MergedTransaction) {
  ...
  return {
    [ChainId.Ethereum]: ChainId.metachainOne,
