...
export interface MergedTransaction {
  // TODO: https://github.com/META-MetaChain/metachain-token-bridge/blob/master/packages/META-token-bridge-ui/src/util/withdrawals/helpers.ts#L31
...
export type AppState = {
  ...
  METATokenBridgeLoaded: boolean
}

export const defaultState: AppState = {
  METATokenBridge: {} as METATokenBridge,
  ...
  sortedTransactions: derived((s: AppState) => {
    const transactions = s.METATokenBridge?.transactions?.transactions || []
  ...
  withdrawalsTransformed: derived((s: AppState) => {
    const withdrawals = Object.values(
      s.METATokenBridge?.pendingWithdrawalsMap || []
