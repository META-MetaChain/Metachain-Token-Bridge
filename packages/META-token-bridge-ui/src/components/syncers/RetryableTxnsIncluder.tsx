...
import { L1ToL2MessageStatus } from '@metachain/sdk'
import {
  EthDepositMessage,
  L1ToL2MessageReader,
  L1ToL2MessageReaderClassic
} from '@metachain/sdk/dist/lib/message/L1ToL2Message'
...
export function RetryableTxnsIncluder(): JSX.Element {
  ...
  const fetchAndUpdateDepositStatus = useCallback(
    async (depositTxId: string, depositAssetType: AssetType | string) => {
      ...
      // Classic messages
      if (isClassic) {
        METATokenBridge?.transactions?.fetchAndUpdateL1ToL2MsgClassicStatus(
          depositTxId,
      ...
      if (isEthDeposit) {
        METATokenBridge?.transactions?.fetchAndUpdateEthDepositMessageStatus(
          depositTxId,
          l1ToL2Msg as EthDepositMessage
        )
      } else {
        // Non-classic - Token deposit
        METATokenBridge.transactions?.fetchAndUpdateL1ToL2MsgStatus(
          depositTxId,
    ...
    [l1Provider, l2Provider, METATokenBridge?.transactions]
  )
  ...
  const checkAndUpdateFailedRetryables = useCallback(async () => {
    ...
    METATokenBridge?.transactions?.addTransactions,
