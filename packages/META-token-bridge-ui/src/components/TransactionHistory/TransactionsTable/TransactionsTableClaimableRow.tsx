...
import { NodeBlockDeadlineStatusTypes } from '../../../hooks/METATokenBridge.types'
...
type CommonProps = {
  ...
  isSourceChainmetachain?: boolean
}
...
function ClaimedTxInfo({ tx, isSourceChainmetachain }: CommonProps) {
  const { l1, l2 } = useNetworksAndSigners()
  const { parentLayer } = useChainLayers()
  const toNetworkId = isSourceChainmetachain ? l1.network.id : l2.network.id
  ...
  return (
    ...
    >
      ...
      <ExternalLink
        ...
        className="META-hover text-blue-link"
      >
...
function ClaimableRowTxID({ tx, isSourceChainmetachain }: CommonProps) {
  const { l1, l2 } = useNetworksAndSigners()
  const { layer } = useChainLayers()
  const fromNetworkId = isSourceChainmetachain ? l2.network.id : l1.network.id
  ...
  return (
    <div className="flex flex-col space-y-3">
      ...
      <ClaimedTxInfo tx={tx} isSourceChainmetachain={isSourceChainmetachain} />
    </div>
  )
}
...
export function TransactionsTableClaimableRow({
  ...
  const { l1, l2 } = useNetworksAndSigners()
  const sourceChainId = tx.cctpData?.sourceChainId ?? ChainId.metachainOne
  const {
    isEthereumMainnetOrTestnet: isSourceChainIdEthereum,
    ismetachain: isSourceChainIdmetachain
  ...
  return (
    ...
    >
      <td className={twMerge('w-1/5 py-3 pl-6 pr-3', customAddressTxPadding)}>
        <ClaimableRowStatus
          tx={tx}
          isSourceChainmetachain={isSourceChainIdmetachain}
        />
      </td>
