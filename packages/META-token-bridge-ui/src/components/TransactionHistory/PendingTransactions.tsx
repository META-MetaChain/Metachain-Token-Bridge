...
const getOtherL2NetworkChainId = (chainId: number) => {
  ...
  return isNetwork(chainId).ismetachainOne
...
export const PendingTransactions = ({
  ...
  const bgClassName = isNetwork(l2Network.id).ismetachainNova
    ? 'bg-gray-dark'
    : 'bg-ocl-blue'
    ...
    >
      ...
      <div className="heading flex items-center justify-between text-base text-white lg:text-lg">
        ...
        {!isNetwork(l2Network.id).isTestnet && (
          <ExternalLink
            className="META-hover cursor-pointer text-sm text-white underline"
            onClick={() => {
