...
import { Erc20Bridger, EthBridger } from '@metachain/sdk'
...
import { AssetType } from '../../hooks/METATokenBridge.types'
...
function useTokenFromSearchParams(): string | undefined {
  const [{ token: tokenFromSearchParams }] = useMETAQueryParams()
...
export function TransferPanel() {
  ...
  const {
    app: {
      ...
      selectedToken,
      isDepositMode,
      METATokenBridgeLoaded,
      METATokenBridge: { eth, token },
      warningTokens
    }
  } = useAppState()
  ...
  const { ismetachainNova } = isNetwork(l2Network.id)

  const latestEth = useLatest(eth)
  const latestToken = useLatest(token)

  const isConnectedTometachain = useLatest(useIsConnectedTometachain())
  ...
  // Both `amount` getter and setter will internally be using `useMETAQueryParams` functions
  const [{ amount }, setQueryParams] = useMETAQueryParams()
  ...
  const transferCctp = async (type: 'deposits' | 'withdrawals') => {
    ...
    const isConnectedToTheWrongChain =
      (isDeposit && isConnectedTometachain.current) ||
      (type === 'withdrawals' && !isConnectedTometachain.current)
    ...
    try {
      if (isDepositMode) {
        ...
        if (warningToken) {
          ...
          return window.alert(
            `${selectedToken?.address} is ${description}; it will likely have unusual behavior when deployed as as standard token to metachain. It is not recommended that you deploy it. (See ${DOCS_DOMAIN}/for-devs/concepts/token-bridge/token-bridge-erc20 for more info.)`
          )
        }
        ...
        if (
          (isConnectedTometachain.current && isParentChainEthereum) ||
        ...
        const isConnectedToEthereum =
          !isConnectedTometachain.current && !isConnectedToOrbitChain.current
        const { isOrbitChain } = isNetwork(l2Network.id)

        if (
          isConnectedToEthereum ||
          (isConnectedTometachain.current && isOrbitChain)
        ) {
          if (shouldTrackAnalytics(l2NetworkName)) {
            trackEvent('Switch Network and Transfer', {
              type: 'Withdrawal',
          ...
          while (
            (!isConnectedTometachain.current &&
              !isConnectedToOrbitChain.current) ||
            (isConnectedTometachain.current && isOrbitChain) ||
            !latestEth.current ||
            !METATokenBridgeLoaded
        ...
        <div
          style={
            isSummaryVisible
              ? {}
              : {
                  background: `url(/images/metachainFaded.webp)`,
