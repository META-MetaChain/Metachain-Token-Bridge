...
export function ClaimableCardConfirmed({ tx }: { tx: MergedTransaction }) {
  ...
  const { ismetachain, isEthereumMainnetOrTestnet } = isNetwork(chainId)
  ...
  const currentChainIsValid = useMemo(() => {
    ...
    return (
      (isSourceChainIdEthereum && ismetachain) ||
      (isSourceChainIdmetachain && isEthereumMainnetOrTestnet)
    )
  }, [
    l2.network.id,
    isSourceChainIdEthereum,
    ismetachain,
    isSourceChainIdmetachain,
            ...
            {isCustomDestinationAddressTx(tx) && (
              <span className="mt-2 flex flex-nowrap gap-1 text-sm text-gray-dark lg:text-base">
                <CustomAddressTxExplorer
                  tx={tx}
                  explorerClassName="META-hover text-blue-link"
                />
              </span>
            )}
          </div>
