...
export function NetworkListbox({
  ...
  const buttonClassName = useMemo(() => {
    ...
    if (ismetachainNova) {
      return 'bg-META-nova-primary'
    }

    return 'bg-META-one-primary'
  }, [value])
  ...
  return (
    ...
    >
      <Listbox.Button
        className={`META-hover flex w-max items-center space-x-1 rounded-full px-3 py-2 text-sm text-white md:text-2xl lg:px-4 lg:py-3 ${buttonClassName}`}
      >
