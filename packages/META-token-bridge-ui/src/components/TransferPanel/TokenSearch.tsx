...
import { METAOneNativeUSDC } from '../../util/L2NativeUtils'
...
const META_ONE_NATIVE_USDC_TOKEN = {
  ...METAOneNativeUSDC,
  ...
  address: CommonAddress.metachainOne.USDC,
  l2Address: CommonAddress.metachainOne.USDC
}

const META_GOERLI_NATIVE_USDC_TOKEN = {
  ...METAOneNativeUSDC,
  listIds: new Set<number>(),
  type: TokenType.ERC20,
  address: CommonAddress.metachainGoerli.USDC,
  l2Address: CommonAddress.metachainGoerli.USDC
}
...
function TokenListsPanel() {
  ...
  const { bridgeTokens, token } = METATokenBridge
  ...
  const listsToShow: BridgeTokenList[] = useMemo(() => {
    ...
    return BRIDGE_TOKEN_LISTS.filter(tokenList => {
      // Don't show the metachain Token token list, because it's special and can't be disabled
      if (tokenList.ismetachainTokenTokenList) {
        return false
      }
...
function TokensPanel({
  ...
  const {
    app: {
      METATokenBridge: { token, bridgeTokens },
      isDepositMode
    }
  } = useAppState()
  const {
  ...
  const { ismetachainOne, ismetachainGoerli } = isNetwork(l2Network.id)
  ...
  const getBalance = useCallback(
    (address: string) => {
      ...
      if (
        isTokenmetachainOneNativeUSDC(address) ||
        isTokenmetachainGoerliNativeUSDC(address)
      ) {
  ...
  const tokensToShow = useMemo(() => {
    ...
    if (!isDepositMode) {
      if (ismetachainOne) {
        tokenAddresses.push(CommonAddress.metachainOne.USDC)
      }
      if (ismetachainGoerli) {
        tokenAddresses.push(CommonAddress.metachainGoerli.USDC)
      }
    }
    const tokens = [
    ...
    return tokens
      .filter(address => {
        ...
        if (isTokenmetachainOneNativeUSDC(address)) {
          // for token search as META One native USDC isn't in any lists
          token = META_ONE_NATIVE_USDC_TOKEN
        }

        if (isTokenmetachainGoerliNativeUSDC(address)) {
          // for token search as META One native USDC isn't in any lists
          token = META_GOERLI_NATIVE_USDC_TOKEN
        }
        ...
        if (!tokenSearch) {
          ...
          // Always show official META token
          if (token?.listIds.has(SPECIAL_metachain_TOKEN_TOKEN_LIST_ID)) {
            return true
          }
          ...
          {({ width }) => (
            <List
              ...
              rowRenderer={virtualizedProps => {
                ...
                if (isTokenmetachainOneNativeUSDC(address)) {
                  token = META_ONE_NATIVE_USDC_TOKEN
                } else if (isTokenmetachainGoerliNativeUSDC(address)) {
                  token = META_GOERLI_NATIVE_USDC_TOKEN
                } else if (address) {
                  token =
...
export function TokenSearch({
  ...
  async function selectToken(_token: ERC20BridgeToken | null) {
    ...
    try {
      ...
      const isNativeUSDC =
        isTokenmetachainOneNativeUSDC(_token.address) ||
        isTokenmetachainGoerliNativeUSDC(_token.address)
    ...
    return (
      <>
        <div className="flex flex-row items-center justify-between pb-4">
          ...
          <button className="META-hover" onClick={close}>
        ...
        <div className="flex justify-end pt-6">
          {isFetchingTokenLists ? (
            ...
            <button
              className="META-hover text-gray text-sm font-medium text-blue-link"
  ...
  return (
    <>
      ...
      <div className="flex justify-start pb-6">
        <button
          className="META-hover flex items-center space-x-2 text-sm font-medium text-blue-link"
