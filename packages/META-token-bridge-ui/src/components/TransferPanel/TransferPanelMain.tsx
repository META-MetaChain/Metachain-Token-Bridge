...
import { ERC20BridgeToken, TokenType } from '../../hooks/METATokenBridge.types'
...
import { GasEstimates } from '../../hooks/METATokenBridge.types'
...
function CustomAddressBanner({
  ...
  const { ismetachain, ismetachainNova } = isNetwork(network.id)
  ...
  const bannerClassName = useMemo(() => {
    ...
    if (ismetachainNova) {
      return 'bg-orange border-META-nova-dark'
    }
    return 'bg-cyan border-META-one-dark'
  }, [ismetachain, ismetachainNova])
...
function NetworkContainer({
  ...
  const { backgroundImage, backgroundClassName } = useMemo(() => {
    ...
    if (ismetachainNova) {
      return {
        backgroundImage: `url('/images/metachainNovaLogo.svg')`,
        backgroundClassName: 'bg-META-nova-dark'
      }
    }

    return {
      backgroundImage: `url('/images/metachainOneLogo.svg')`,
      backgroundClassName: 'bg-META-one-dark'
    }
  }, [network])
...
export function TransferPanelMain({
  ...
  const { ismetachainOne, ismetachainGoerli } = isNetwork(l2.network.id)
  ...
  const { METATokenBridge, isDepositMode, selectedToken } = app
  const { token } = METATokenBridge
  ...
  useEffect(() => {
    ...
    if (
      ...
      isTokenmetachainOneNativeUSDC(selectedToken.address) ||
      isTokenmetachainGoerliNativeUSDC(selectedToken.address)
    ) {
  ...
  const [externalFrom, externalTo] = useMemo(() => {
    const isParentChainmetachain = isNetwork(l1.network.id).ismetachain

    if (isParentChainmetachain) {
      return isConnectedTometachain
  ...
  const showUSDCSpecificInfo =
    (isTokenMainnetUSDC(selectedToken?.address) && ismetachainOne) ||
    (isTokenGoerliUSDC(selectedToken?.address) && ismetachainGoerli)

  const [, setQueryParams] = useMETAQueryParams()
  ...
  const setMaxAmount = useCallback(async () => {
    ...
    try {
      ...
      const estimatedL1GasFees = calculateEstimatedL1GasFees(
        ...
        // https://github.com/META-MetaChain/metachain-docs/blob/1bd3b9beb0858725d0faafa188cd13d32f642f9c/metachain-docs/devs-how-tos/how-to-estimate-gas.mdx#L125
  ...
  useEffect(() => {
    const isMETAOneUSDC = isTokenmetachainOneNativeUSDC(selectedToken?.address)
    const isMETAGoerliUSDC = isTokenmetachainGoerliNativeUSDC(
    ...
    if (isMETAOneUSDC) {
      ...
      actions.app.setSelectedToken({
        ...
        l2Address: CommonAddress.metachainOne['USDC.e']
      })
    } else if (isMETAGoerliUSDC) {
      token.updateTokenData(CommonAddress.Goerli.USDC)
      actions.app.setSelectedToken({
        ...commonUSDC,
        address: CommonAddress.Goerli.USDC,
        l2Address: CommonAddress.metachainGoerli['USDC.e']
      })
    }
  ...
  const networkListboxProps: NetworkListboxesProps = useMemo(() => {
    ...
    function modifyOptions(selectedChainId: ChainId, direction: 'from' | 'to') {
      ...
      const { ismetachain: isSelectedmetachainChain } = isNetwork(selectedChainId)
      ...
      return [l1.network, ...options].filter(option => {
        ...
        // and the selected destination is an metachain chain
    ...
    function shouldOpenOneNovaDialog(selectedChainIds: number[]) {
      return [ChainId.metachainOne, ChainId.metachainNova].every(chainId =>
      ...
      return {
        from: {
          ...
          onChange: async network => {
            ...
            try {
              ...
              // Pair Ethereum with an metachain chain if an Orbit chain is currently selected.
        ...
        to: {
          ...
          onChange: async network => {
            ...
            // If Orbit chain is selected, we want to change network to metachain One or metachain Goerli.
            if (isOrbitChain) {
              try {
                if (!isConnectedTometachain) {
        ...
        onChange: async network => {
          ...
          if (isEthereumMainnetOrTestnet) {
            if (isConnectedToOrbitChain) {
              try {
                // If connected to an Orbit chain, we need to change network to metachain.
            ...
            // Connected to metachain.
            updatePreferredL2Chain(defaultPartnerChain)
          }

          if (isOrbitChain) {
            const isConnectedToEthereum =
              !isConnectedTometachain && !isConnectedToOrbitChain

            if (isConnectedToEthereum) {
              // If connected to Ethereum, we need to change network to metachain.
            ...
            if (isOrbitChainCurrentlySelected) {
              ...
              // right now only metachain Goerli is support so it's fine
              await switchNetworkAsync?.(defaultPartnerChain)
            }
  ...
  return (
    <div className="flex flex-col px-6 py-6 lg:min-w-[540px] lg:px-0 lg:pl-6">
            ...
            {isSwitchingL2Chain ? (
              ...
              utils.isAddress(destinationAddressOrWalletAddress) && (
                <>
                  ...
                  the UI shows the META One balance of both USDC.e and native USDC */}
                  {app.isDepositMode && showUSDCSpecificInfo && (
                    <TokenBalance
                      balance={
                        (ismetachainOne
                          ? erc20L2Balances?.[CommonAddress.metachainOne.USDC]
                          : erc20L2Balances?.[
      ...
      >
        <p>
          The metachain bridge does not currently support {selectedToken?.symbol}
