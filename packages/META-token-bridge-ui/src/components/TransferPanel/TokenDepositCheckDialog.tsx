...
export function TokenDepositCheckDialog(props: TokenDepositCheckDialogProps) {
  ...
  const textContent = useMemo(() => {
    switch (type) {
      case 'user-added-token':
        return (
          <>
            You are about to deposit {symbol} to metachain 🎉 <br />
            <br />
            ...
            <ul>
              <li>
                •{' '}
                <a
                  href={`${DOCS_DOMAIN}/for-devs/concepts/token-bridge/token-bridge-erc20#the-metachain-generic-custom-gateway`}
                  target="_blank"
      ...
      case 'new-token':
        return (
          <>
            You are the first to bridge {symbol} to metachain 🎉 <br />
            <br />
            <b>Important facts</b>
            <ol>
  ...
  const title = useMemo(() => {
    switch (type) {
      case 'user-added-token':
        return `Depositing ${symbol} to metachain`
